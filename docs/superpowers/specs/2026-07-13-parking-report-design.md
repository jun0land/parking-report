...
- **디스크 쿼터 주의(PythonAnywhere 무료 플랜 512MB)**: Flask 가상환경만으로도 300MB 이상을 차지할 수 있으므로 `requirements.txt`는 위 목록 이상으로 늘리지 않는다. 시드 데이터의 샘플 이미지는 소수·저용량으로 유지하고, 모든 업로드 이미지는 저장 전 최대 1280px로 리사이즈해 디스크 사용량을 통제한다(10번 항목과 연동).
...
72시간 동안 매칭되지 않은 PENDING 사진은 조회 시점에 `EXPIRED`로 표시한다(별도 백그라운드 작업 없이 lazy evaluation). 이 `status` 필드는 UI 표시 전용이며, **AI 판독 점수 계산(6번)에서 "만료된 반복 방문 이력"을 셀 때는 `status='EXPIRED'`가 아니라 `captured_at < now - 72h`를 직접 조건으로 사용**한다. lazy evaluation 특성상 아무도 조회하지 않으면 DB상 status가 여전히 PENDING으로 남아있을 수 있어, status 필드에 의존하면 점수 계산이 과소 집계되기 때문이다.
...
- 동일 번호판+동일 위치 조합에서 과거에 매칭되지 못한 채 72시간이 지난(`captured_at < now - 72h`인, status 필드와 무관하게 시각 기준으로 판정) PENDING 사진이 여러 건 있으면 감점 (짧은 정차를 반복하는 패턴 — 하차 등 반복 방문 신호)
...
- `/login`, `/logout`
- `/upload` — 사진 업로드, 번호판 수동 입력(코드 주석: 실서비스는 Vision AI ANPR로 대체), EXIF 자동 추출 실패 시 위도/경도·촬영시각 수동 입력 폼 노출. 폰/브라우저 업로드 시 EXIF가 제거되는 경우가 흔하므로 수동 입력 경로는 에러가 아니라 정상 플로우로 안내한다(예: "위치 정보를 찾을 수 없어요 — 아래에 직접 입력해주세요" 톤). 데모 계정으로 접속했을 때는 매칭 조건("업로더가 서로 다름")을 충족시키기 위해, 시드로 심어둔 상대방 PENDING 사진과 정확히 일치하는 번호판·좌표를 안내 배너로 미리 제시한다(예: "데모 시연: 번호판 12가3456, 좌표 OOO를 입력해보세요 — 다른 시민이 방금 올린 사진과 바로 매칭됩니다").
...
- **DEPLOY.md에 "심사 기간 갱신 체크리스트"를 반드시 포함**: PythonAnywhere 무료 플랜은 2026-01-15 이후 생성된 계정 기준 웹앱이 1개월 후 자동 만료된다. 심사가 여러 주에 걸쳐 진행되거나 제출 후 뒤늦게 링크를 열어볼 수 있으므로, (1) 제출 직전 Web 탭에서 앱을 Reload해 만료 시점을 미루고, (2) 심사 기간 중 주기적으로(예: 2주 간격) 재접속/Reload하는 절차를 체크리스트로 명시한다.
- `scripts/seed_data.py`:
  - 여러 유저(각기 다른 신뢰도 점수대 포함)
  - 여러 행정동
  - 매칭 직전 상태의 PENDING 사진(같은 번호판, 같은 위치, 아직 짝이 없는 상태) — **반드시 데모 계정(`is_demo=True`)이 아닌 다른 시드 유저 소유로 생성**하여, 데모 계정으로 로그인해 업로드하는 순간 실제로 매칭이 발생하도록 보장한다. 이 PENDING 사진의 번호판·좌표는 `/upload` 안내 배너에 그대로 노출되는 값과 일치시킨다.
  - 이미 VALID / REJECTED / FALSE로 처리된 과거 이력 (대시보드가 바로 그럴듯하게 보이도록)
  - 데모 계정 1개는 `is_demo=True`로 지정하여 "둘러보기" 버튼과 연결
  - 시드 이미지는 디스크 쿼터 보호를 위해 소수·저용량 샘플만 사용한다(2번 항목과 연동)

## 11. 구현 중 결정해도 되는 사항

다음은 스펙 승인 단계에서 확정하지 않고, 구현 시점에 실용적으로 결정한다:
- 같은 차량 사진 3장이 모인 경우(2장이 이미 매칭된 뒤 3번째가 PENDING으로 남는 처리)
- 신뢰도 점수 상한(100점 초과 허용 여부)
- 일일 업로드 제한의 "하루" 경계 기준(KST 자정 리셋 등)
